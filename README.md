...
Publisher-controlled AI key takeaways for WordPress. SummarAIze helps publishers generate, edit, bulk-manage, and display editor-approved summaries using their own OpenAI, Google Gemini, or OpenRouter API key.
...
- Use your own OpenAI, Google Gemini, or OpenRouter account, including GPT-6 Astra, and control your model spend
...
Choose **OpenRouter** in Settings > SummarAIze and enter your API key. Click **Load models**, filter by provider or model name, and select a result. The selected model ID remains editable for custom models.

Click **Test model** to generate five sample takeaways using the same request and response validation as a real summary. The preview lets you review the result before using it on a post. Tests may incur a small API charge. The badge shows **Untested**, **Test passed**, or **Test failed** with a specific error. Results apply to the tested key/model pair for seven days; changing either input requires another test. A successful sample does not guarantee every article will succeed.

Click **Save Changes** to retain your key and selected model. Searching or testing alone does not save those settings.

Your content passes through OpenRouter to the model provider. Your account privacy settings, model availability, and credit limits apply. Model requests must return five valid takeaways before the plugin saves them.

## Upgrading to 1.5.0

Existing provider selections, API keys, OpenAI model choices, display settings, and saved summaries are retained. OpenRouter is optional; switching providers does not erase the other providers' saved credentials. GPT-5 mini remains the default for new OpenAI configurations; GPT-6 Astra is an optional selection.

Gemini now uses Gemini 3.5 Flash-Lite because the previously hard-coded Gemini 2.5 Flash-Lite is unavailable to new users. Key validation no longer generates billable content or labels a retired-model error as an invalid key. Existing saved summaries are not regenerated by the upgrade.

## Requirements

- WordPress 5.0 or newer, tested through WordPress 7.1
- PHP 7.2 or newer
- An OpenAI, Google Gemini, or OpenRouter API key
...
4. Choose OpenAI, Google Gemini, or OpenRouter and save your API key.
